...
for i ∈ 1:number_of_samples
    ...
    CSV.write(path_to_synthetic_ensemble, Tables.table(ensemble_archive); header=["parameters"])
    
    # dump output to disk -
    data_output = [Y Yₘ]
    data_output_header = ["actual", "simulated"]
    path_to_synthetic_patient_otuput = joinpath(_PATH_TO_TMP, "OUT-Synthetic-1k-P$(i).csv")
    CSV.write(path_to_synthetic_patient_otuput, Tables.table(data_output); header = data_output_header)
    ...
    CSV.write(path_to_synthetic_patient_simulation, Tables.table(data_state); header=data_state_header)
